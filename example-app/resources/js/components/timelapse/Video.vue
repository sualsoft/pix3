...
const videos = ref([]);
const sectionTitle = ref('Notre portefeuille');
...
const fetchData = async () => {
    try {
        ...
        if (data.timelapse_videos) {
            sectionTitle.value = data.timelapse_videos.title;
            videos.value = data.timelapse_videos.videos;
        }
    } catch (error) {
        console.error(error);
    }
        ...
        <div class="mx-auto max-w-7xl">
            ...
            >
                <div
                    v-for="n in 3"
                    :key="n"
                    class="aspect-video animate-pulse bg-gray-100"
                ></div>
            </div>
