    ...
    try {
        ...
        if (response.ok) {
            ...
            if (result.errors) {
                ...
                message.value = '❌ Error: ' + (result.message || 'Failed to add file');
            }
        }
    } catch (error) {
...
const deleteFile = async (fileId) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            message.value = '❌ Error: ' + (result.message || 'Failed to delete file');
        }
    } catch (error) {
        message.value = '❌ Network error: ' + error.message;
...
<template>
    <div class="space-y-6">
        <!-- Status Message -->
        <div v-if="message" class="rounded-lg p-4" :class="{
        ...
        <div class="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-xl">
            <div class="bg-gradient-to-r from-indigo-600 to-purple-700 p-6 text-white">
                ...
                <p class="mt-1 text-indigo-100">Add new content to {{ projectName }}</p>
            </div>
            ...
            <div class="p-6 md:p-8">
                <div class="space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Source du téléchargement</label>
                        
                        <div class="space-y-3">
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">Téléverser un fichier</label>
                                <input
                                    type="file"
                            ...
                            <div>
                                <label class="block text-xs text-gray-500 mb-1">Lien Google Drive / Externe</label>
                                <input
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">ID YouTube (Pour l'aperçu)</label>
                        <input
                        ...
                        />
                        <p class="mt-1 text-xs text-gray-500">Laissez vide si vous ne voulez pas de lecture directe.</p>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Miniature (Image)</label>
                        <input
                            type="file"
                            accept="image/*"
                    ...
                    <div class="flex justify-end">
                        ...
                        >
                            {{ isLoading ? 'Ajout en cours...' : 'Ajouter le Fichier' }}
                        </button>
                    </div>
                </div>
            </div>
                    ...
                    >
                        <div class="flex justify-between">
                            <div>
                                ...
                                <p class="text-sm text-gray-600 capitalize">{{ file.type }}</p>
                                <p class="text-xs text-gray-500 mt-1">{{ file.date }}</p>
                            </div>
                            <div class="text-right">
                                <span v-if="file.badge" class="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-medium text-blue-800">
                                    {{ file.badge }}
                                </span>
                                <button
                        ...
                        <div v-if="file.thumbnail" class="mt-3">
                            <img :src="file.thumbnail" :alt="file.name" class="h-20 w-20 rounded object-cover">
                        </div>
                    </div>
                </div>
                <div v-else class="text-center text-gray-500 py-8">
                    <p>Aucun fichier trouvé.</p>
                    <p class="mt-2 text-sm">Ajoutez votre premier fichier en utilisant le formulaire ci-dessus.</p>
                </div>
            </div>
        </div>
    </div>
