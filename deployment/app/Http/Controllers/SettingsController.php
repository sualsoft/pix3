    ...
    public function updateWhy(Request $request)
{
    ...
    // Add this new method for contact SEO content
    public function updateContactSeo(Request $request)
    {
        ...
        if ($request->hasFile('og_image')) {
            ...
            $ogImageName = 'contact-og-' . uniqid() . '.' . $ogImage->getClientOriginalExtension();
            $ogImage->move(public_path('images/seo'), $ogImageName);
            $ogImagePath = '/images/seo/' . $ogImageName;
        }

        // Handle Twitter image upload
        if ($request->hasFile('twitter_image')) {
            $twitterImage = $request->file('twitter_image');
            $twitterImageName = 'contact-twitter-' . uniqid() . '.' . $twitterImage->getClientOriginalExtension();
            $twitterImage->move(public_path('images/seo'), $twitterImageName);
            $twitterImagePath = '/images/seo/' . $twitterImageName;
        }

        // 3. Get existing setting to preserve existing images if not updated
        $existingSetting = SiteSetting::where('key', 'contact_seo')->first();
        $existingContent = $existingSetting ? $existingSetting->content : [];

        // 4. Prepare content array with all SEO fields
        $content = [
            ...
            'og_image' => $ogImagePath ?? ($request->og_image ?? ($existingContent['og_image'] ?? '')),
            'og_type' => $request->og_type ?? 'website',
            'twitter_card' => $request->twitter_card ?? 'summary_large_image',
            'twitter_title' => $request->twitter_title ?? '',
            'twitter_description' => $request->twitter_description ?? '',
            'twitter_image' => $twitterImagePath ?? ($request->twitter_image ?? ($existingContent['twitter_image'] ?? '')),
        ];
        ...
        // 6. Send back success
        return response()->json(['message' => '✅ Contenu SEO mis à jour avec succès !']);
    }

    // Add method for portfolio SEO content
    public function updatePortfolioSeo(Request $request)
    {
        // 1. Validate
        $request->validate([
            'content' => 'nullable|string',
        ...
        if ($request->hasFile('og_image')) {
            $ogImage = $request->file('og_image');
            $ogImageName = 'portfolio-og-' . uniqid() . '.' . $ogImage->getClientOriginalExtension();
            $ogImage->move(public_path('images/seo'), $ogImageName);
            $ogImagePath = '/images/seo/' . $ogImageName;
        }
        ...
        if ($request->hasFile('twitter_image')) {
            $twitterImage = $request->file('twitter_image');
            $twitterImageName = 'portfolio-twitter-' . uniqid() . '.' . $twitterImage->getClientOriginalExtension();
            $twitterImage->move(public_path('images/seo'), $twitterImageName);
            $twitterImagePath = '/images/seo/' . $twitterImageName;
        }

        // 3. Get existing setting to preserve existing images if not updated
        $existingSetting = SiteSetting::where('key', 'portfolio_seo')->first();
        $existingContent = $existingSetting ? $existingSetting->content : [];
    ...
    // Add method for drone SEO content
    public function updateDroneSeo(Request $request)
    {
        // 1. Validate
        $request->validate([
            'content' => 'nullable|string',
        ...
        if ($request->hasFile('og_image')) {
            $ogImage = $request->file('og_image');
            $ogImageName = 'drone-og-' . uniqid() . '.' . $ogImage->getClientOriginalExtension();
            $ogImage->move(public_path('images/seo'), $ogImageName);
            $ogImagePath = '/images/seo/' . $ogImageName;
        }
        ...
        if ($request->hasFile('twitter_image')) {
            $twitterImage = $request->file('twitter_image');
            $twitterImageName = 'drone-twitter-' . uniqid() . '.' . $twitterImage->getClientOriginalExtension();
            $twitterImage->move(public_path('images/seo'), $twitterImageName);
            $twitterImagePath = '/images/seo/' . $twitterImageName;
        }

        // 3. Get existing setting to preserve existing images if not updated
        $existingSetting = SiteSetting::where('key', 'drone_seo')->first();
        $existingContent = $existingSetting ? $existingSetting->content : [];
    ...
    // Add method for timelapse SEO content
    public function updateTimelapseSeo(Request $request)
    {
        // 1. Validate
        $request->validate([
            'content' => 'nullable|string',
        ...
        if ($request->hasFile('og_image')) {
            $ogImage = $request->file('og_image');
            $ogImageName = 'timelapse-og-' . uniqid() . '.' . $ogImage->getClientOriginalExtension();
            $ogImage->move(public_path('images/seo'), $ogImageName);
            $ogImagePath = '/images/seo/' . $ogImageName;
        }
        ...
        if ($request->hasFile('twitter_image')) {
            $twitterImage = $request->file('twitter_image');
            $twitterImageName = 'timelapse-twitter-' . uniqid() . '.' . $twitterImage->getClientOriginalExtension();
            $twitterImage->move(public_path('images/seo'), $twitterImageName);
            $twitterImagePath = '/images/seo/' . $twitterImageName;
        }

        // 3. Get existing setting to preserve existing images if not updated
        $existingSetting = SiteSetting::where('key', 'timelapse_seo')->first();
        $existingContent = $existingSetting ? $existingSetting->content : [];
    ...
    // Add method for home SEO content
    public function updateHomeSeo(Request $request)
    {
        ...
        if ($request->hasFile('og_image')) {
            $ogImage = $request->file('og_image');
            $ogImageName = 'home-og-' . uniqid() . '.' . $ogImage->getClientOriginalExtension();
            $ogImage->move(public_path('images/seo'), $ogImageName);
            $ogImagePath = '/images/seo/' . $ogImageName;
        }
        ...
        if ($request->hasFile('twitter_image')) {
            $twitterImage = $request->file('twitter_image');
            $twitterImageName = 'home-twitter-' . uniqid() . '.' . $twitterImage->getClientOriginalExtension();
            $twitterImage->move(public_path('images/seo'), $twitterImageName);
            $twitterImagePath = '/images/seo/' . $twitterImageName;
        }

        // 3. Get existing setting to preserve existing images if not updated
        $existingSetting = SiteSetting::where('key', 'home_seo')->first();
        $existingContent = $existingSetting ? $existingSetting->content : [];
